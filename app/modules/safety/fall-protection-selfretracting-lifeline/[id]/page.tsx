...
const fallProtectionSRLSchema = z.object({
  formName: z.string().default("Fall-Protection-Selfretracting-Lifeline"),
  pdfName: z.string().default("FallProtectionSelfretractingLifeline.pdf"),
...
type FallProtectionSRLForm = z.infer<typeof fallProtectionSRLSchema>;
...
export default function FallProtectionSRLPage() {
  ...
  const methods = useForm<FallProtectionSRLForm>({
    resolver: zodResolver(fallProtectionSRLSchema),
    defaultValues: {
      formName: "Fall-Protection-Selfretracting-Lifeline",
      pdfName: "FallProtectionSRL.pdf",
      ...
      formData: {
        ...
        housingConnectors: {
          ...
          nutsBolts: { status: "not_checked", note: "" },
          evidenceOfDamage: { status: "not_checked", note: "" },
          connector: { status: "not_checked", note: "" },
          pitting: { status: "not_checked", note: "" },
        },
        lifeline: {
          entireLengthRetracts: { status: "not_checked", note: "" },
          testBraking: { status: "not_checked", note: "" },
          termination: { status: "not_checked", note: "" },
          excessiveWearKinks: { status: "not_checked", note: "" },
          cutsFraying: { status: "not_checked", note: "" },
          cableSeparating: { status: "not_checked", note: "" },
  ...
  const saveDraft = useCallback(
    async (data: FallProtectionSRLForm) => {
      try {
  ...
  const loadFallProtectionSRL = useCallback(
    async (formId: string) => {
      try {
        ...
        const result = await response.json();
        console.log(
          "Fall Protection Self Retracting Lifeline API response:",
          result
        );
        ...
        // Verify this is actually a self retracting lifeline inspection
        if (form.formName !== "Fall-Protection-Selfretracting-Lifeline") {
          throw new Error(
            "This form is not a self retracting lifeline inspection"
          );
        }
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        await loadJobs();
        if (id && id !== "new") {
          await loadFallProtectionSRL(id);
        } else {
          setStatus("success");
        }
      } catch (err) {
    ...
    fetchData();
  }, [id, loadFallProtectionSRL, loadJobs]);

  // ---------------- Form Submission ----------------

  const onSubmit = async (formData: FallProtectionSRLForm) => {
    try {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-white rounded-md shadow-md">
      <h1 className="text-2xl font-semibold text-gray-700 mb-6">
        {id && id !== "new"
          ? "Edit Fall Protection Self Retracting Lifeline"
          : "New Fall Protection Self Retracting Lifeline"}
      </h1>
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  <InspectionItem
                    ...
                    fieldPath="formData.housingConnectors.coverShrinkTube"
                    register={register}
                  />
                  <InspectionItem
                    label="Nuts / Bolts / Hook Body / Rivets / Screws"
                    fieldPath="formData.housingConnectors.nutsBolts"
                    register={register}
                  />
                  <InspectionItem
                    label="Evidence of Damage (Dents, Cracks, Rust)"
                    fieldPath="formData.housingConnectors.evidenceOfDamage"
                    register={register}
                  />
                  <InspectionItem
                    label="Connector (Self Closing & Locking)"
                    fieldPath="formData.housingConnectors.connector"
                    register={register}
                  />
                  <InspectionItem
                    label="Pitting / Nicks / Corrosion"
                    fieldPath="formData.housingConnectors.pitting"
                    register={register}
                  />
                </tbody>
              </table>
          ...
          <div className="bg-gray-50 p-4 rounded-md mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                ...
                <tbody>
                  <InspectionItem
                    label="Entire Length Retracts Smoothly"
                    fieldPath="formData.lifeline.entireLengthRetracts"
                    register={register}
                  />
                  <InspectionItem
                    label="Test Braking / Locking Function"
                    fieldPath="formData.lifeline.testBraking"
                    register={register}
                  />
                  <InspectionItem
                    label="Termination (Stitch, Splice or Swage)"
                    fieldPath="formData.lifeline.termination"
                    register={register}
                  />
                  <InspectionItem
                    label="Excessive Wear / Kinks"
                    fieldPath="formData.lifeline.excessiveWearKinks"
                    register={register}
                  />
                  <InspectionItem
                    label="Cuts / Fraying / Broken Stitching"
                    fieldPath="formData.lifeline.cutsFraying"
                    register={register}
                  />
                  <InspectionItem
                    label="Cable Separating / Bird-Caging"
                    fieldPath="formData.lifeline.cableSeparating"
                    register={register}
                  />
                  <InspectionItem
                    label="Excessive Wear"
                    fieldPath="formData.lifeline.excessiveWear"
                    register={register}
                  />
                </tbody>
              </table>
            ...
            <FormField label="">
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="generatePDF" className="text-gray-700">
                  Generate PDF for this Fall Protection Self Retracting Lifeline
                </label>
              </div>
            </FormField>
          )}
